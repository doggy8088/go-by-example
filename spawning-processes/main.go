...
func main() {
	...
	// .Output is another helper that handles the common case of running a command, waiting for it to finish,
	// and collecting its output. If there were no errors, dateOut will hold bytes with the date info.
	...
	// Next we'll look at a slightly more involved case where we pipe data to the
	...
	// if err != nil pattern for all of them. We also only collect the StdoutPipe results, but you could collect the StderrPipe in exactly the same way.
